...
class DemoColegiadoSeeder extends Seeder
{
    public function run(): void
    {
        ...
        for ($i = 1; $i <= 12; $i++) {
            CollegiateDue::create([ 'collegiate_id' => $perez->id, 'amount' => 5000.00, 'due_date' => Carbon::now()->subMonths($i)->day(10), 'status' => 'overdue' ]);
        }
        ...
        EthicsSanction::create([ 'collegiate_id' => $gomez->id, 'type' => 'temporary', 'reason' => 'Falta de ética profesional (Exp. 445/2026)', 'status' => 'active', 'start_date' => Carbon::now()->subDays(15), 'end_date' => Carbon::now()->addDays(45) ]);
        ...
        $agreement = PaymentAgreement::create([ 'school_id' => $school->id, 'collegiate_id' => $lopez->id, 'type' => 'yearly_promo', 'total_amount_original' => 60000.00, 'total_amount_agreement' => 50000.00, 'installment_count' => 12, 'status' => 'active', 'metadata' => ['promo_name' => 'Plan Anual 12x10'] ]);

        for ($i = 0; $i < 12; $i++) {
            PaymentAgreementInstallment::create([ 'payment_agreement_id' => $agreement->id, 'due_date' => Carbon::now()->addMonths($i)->day(15), 'amount' => 4166.66, 'status' => $i < 2 ? 'paid' : 'pending' ]);
        }

        // Crear masivos usando el helper global \fake()
        for ($i = 0; $i < 100; $i++) {
            $regNum = 'MAT-' . (5000 + $i);
            $c = Collegiate::create([
                'school_id' => $school->id,
            ...
            CollegiateDue::create([ 'collegiate_id' => $c->id, 'amount' => 5000, 'due_date' => Carbon::now()->subMonth()->day(10), 'status' => $c->is_fees_compliant ? 'paid' : 'overdue' ]);
        }
    }
}
