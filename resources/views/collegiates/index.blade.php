                    ...
                    <tr class="border-bottom border-light table-row-main" id="row-{{ $col->id }}">
                        <td class="py-2 px-4 searchable" data-field="name">
                            <div class="d-flex align-items-center gap-3">
                                <div>
                                    @if($col->avatar_url)
                                        <img src="{{ $col->avatar_url }}" alt="Avatar" class="rounded-circle object-fit-cover shadow-sm" style="width: 97px; height: 97px; border: 2px solid var(--bs-primary);">
                                    @else
                                        <div class="rounded-circle bg-light d-flex align-items-center justify-content-center shadow-sm border border-secondary border-opacity-25" style="width: 97px; height: 97px;">
                                            <span class="text-secondary fw-bold" style="font-size: 2.0rem;">{{ strtoupper(substr($col->first_name, 0, 1) . substr($col->last_name, 0, 1)) }}</span>
                                        </div>
                                    @endif
                                </div>
